...
def main():
    print("====================================================")
    print("Kicking off SEASONAL Constructed Analog Run")
    print("====================================================")
    ...
    for dtype in config.DATASET_TYPES:
        ...
        out_filename = config.OUT_DATA_DIR / f"{dtype}.seasonal.eof_modes_master.nc"
        master_eof_outputs.to_netcdf(out_filename)
        
        print(f"SUCCESS: Master spectral matrix written to {out_filename.name}")
        print(f"--- Completed Seasonal Track for {dtype.upper()} ---\n")
